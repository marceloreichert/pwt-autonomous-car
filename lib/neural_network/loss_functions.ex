defmodule AutonomousCar.NeuralNetwork.LossFunctions do
  import Math
  import Nx.Defn
  ...
  def l1_loss(target_value, predict_value) do
    abs(target_value - predict_value)
  end

  # MSE Loss
  defn loss(preds, target) do
    Nx.mean(Nx.power(target - preds, 2))
  end
  ...
  def smooth_l1_loss(target_value, predict_value) do
    with l1_loss_value <- l1_loss(target_value, predict_value) do
      case l1_loss_value do
        _value when _value < 1 -> squared_error(0, target_value, predict_value) / 1
